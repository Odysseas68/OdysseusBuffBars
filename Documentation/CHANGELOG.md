...
- Kept LEFT/RIGHT/ABOVE and arbitrary graph/cycle parity, managed `anchorsShown`, future full ENCHANTMENTS bounds, remaining behavior/filter settings, legacy-only ENCHANTMENTS control cleanup, direct OffHand/both-slot transition coverage, production cutover, rollback policy, and eventual comparison/legacy cleanup pending.
