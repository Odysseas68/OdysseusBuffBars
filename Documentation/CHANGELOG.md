...
- Added runtime-only group renderer authority with defaults BUFFS `LEGACY`, DEBUFFS `MANAGED`, and ENCHANTMENTS `LEGACY`. Authority is intentionally absent from SavedVariables, defaults, and migrations, so `/reload` restores the table.
- Completed the first staged production cutover with managed DEBUFFS authoritative. The shared legacy group refresh now serves both `RefreshAll()` and player `UNIT_AURA`: B/E retain normal scan/render behavior, while managed-authoritative D skips `Engine:Scan` and real-data `Bars:UpdateGroup` work.
- Added explicit retained-frame cleanup for stale legacy DEBUFFS rows: remove timer `OnUpdate`, hide tooltips, clear cancel state out of combat, clear row data/elapsed state and timer text, reset/hide the status bar, and hide pooled rows without deleting them.
- Made legacy visibility authority-aware. Existing B/E `showLegacyBars` and comparison behavior is preserved; D stays hidden under managed authority regardless of legacy visibility, comparison, or anchor settings, and returns to ordinary legacy semantics after rollback.
- Added out-of-combat session rollback through `SetGroupRendererAuthority(2, "LEGACY")` and restoration through `SetGroupRendererAuthority(2, "MANAGED")`. Managed D is disabled/hidden but retained during rollback; a fresh legacy D scan is performed. Returning to managed clears legacy D before re-enabling the existing managed host/container.
- Preserved the mixed-renderer topology invariant: managed BUFFS remains shown, enabled, and layout-active as the managed DEBUFFS BELOW/RIGHT/LEFT geometry source even though BUFFS remains legacy-authoritative for production.
...
- At the PTR checkpoint, managed DEBUFFS/ENCHANTMENTS production integration, final configuration parity, Blizzard BuffFrame visibility policy, and production cutover remained incomplete. Later Live work below finalizes the group-specific configuration policy and Edit Mode visibility conclusion; the current checkpoint completes DEBUFFS production authority while ENCHANTMENTS remains pending.
...
- At that checkpoint, kept arbitrary graph/cycle parity, BUFFS-as-child, future full ENCHANTMENTS bounds, ABOVE cutover handling, the final parity audit, opportunistic direct OffHand/both-slot coverage, broader native lifecycle coverage, production cutover, rollback policy, release preparation, and eventual comparison/legacy cleanup pending. Later entries complete the final audit and DEBUFFS production authority/rollback; BUFFS and ENCHANTMENTS cutovers plus the other listed work remain separate.
