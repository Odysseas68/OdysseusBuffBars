...
At startup, all three managed groups consume `name`, width, height, spacing, font size, complete bar/background RGBA, icon side, scale, alpha, and growth direction. BUFFS and DEBUFFS also consume compatible saved sort and maximum-bar settings. ENCHANTMENTS deliberately retains its separate prototype sort/cap behavior because one legacy value cannot exactly govern `HelpfulEnhancements`, native item-enchantment rows, and the ordinary fishing-lure row together.
...
Visual parity and the expanded configuration checkpoint are runtime validated for live icon side, scale/alpha, BUFFS/DEBUFFS saved sort and maximum counts, and BUFFS and ENCHANTMENTS growth direction in addition to the earlier presentation slice. DEBUFFS uses the same supported growth implementation without equivalent direct real-HARMFUL coverage. Host placement/position, remaining behavior/filter settings, managed position persistence, and production integration remain pending.
...
- A known real private harmful aura, optional explicit secrecy/`NeverSecret` tests, the DEBUFFS filtering product decision, broader native item-enchantment/effect coverage, placement/position, remaining behavior/filter integration, legacy-only ENCHANTMENTS control cleanup, persistence, production cutover, rollback policy, and removal of the legacy scanner remain future work.
