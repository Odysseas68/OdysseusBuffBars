...
The managed player-BUFFS prototype is PTR validated for its core lifecycle, presentation, self-sizing layout, native sorting, whitelist/blacklist filtering, automatic filter-editor synchronization, combat updates, and native interaction. Core managed player-DEBUFFS runtime behavior is validated on Retail Live `12.1.0.69273`. Core managed MainHand lifecycle, dynamic FOOD/FLASK_PHIAL/AUGMENT_RUNE/FISHING_BOBBER routing, and the fishing-lure apply/expire/reapply lifecycle are also validated on Retail Live. Visual comparison against the legacy renderer and live OOC font/color/width/height/spacing synchronization now pass for all three managed areas. They remain parallel prototypes rather than production backends.
...
Live out-of-combat synchronization currently covers `fontSize`, complete `barColor`, complete `barBgColor`, `width`, `height`, and `spacing`. Existing and future/reused managed rows use prototype-owned current state; width also updates headers, height updates square icons and the `height + iconGap` colored boundary, and spacing updates public managed layout descriptors plus the ordinary lure gap. Replacement layouts preserve current width/height/spacing together through `SetAuraGroupLayout` and `SetItemEnchantmentLayout`. The managed container remains self-sizing, and no active children, counts, or aura identities are inspected.

Managed `iconSide` remains startup/reload-only even though the legacy renderer applies it live. Scale, alpha, header text, sort, maximum bars, placement/position, chaining policy, growth, timed/timeless behavior, and broader filter/override integration remain startup-only or pending as applicable. The existing config layer remains authoritative for SavedVariables and `syncGroupBars`; the managed backend only consumes the resulting supported values during centralized apply.
...
- A known real private harmful aura, optional explicit secrecy/`NeverSecret` tests, the DEBUFFS filtering product decision, broader native item-enchantment/effect coverage, live icon-side reanchoring, scale/alpha, growth, placement/position, remaining behavior/filter integration, persistence, production cutover, rollback policy, and removal of the legacy scanner remain future work.
