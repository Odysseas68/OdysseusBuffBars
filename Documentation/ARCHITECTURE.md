...
Visual parity and the expanded configuration checkpoint are runtime validated for live icon side, scale/alpha, BUFFS/DEBUFFS saved sort and maximum counts, BUFFS and ENCHANTMENTS growth direction, supported SCREEN/BELOW combinations, independent SCREEN-root dragging/persistence, lock/combat handling, reset/reload behavior, and managed-only/overlapping/side-by-side development presentation. DEBUFFS uses the same supported growth implementation without equivalent direct real-HARMFUL coverage. LEFT/RIGHT/ABOVE, arbitrary graph/cycle parity, managed `anchorsShown`, remaining behavior/filter settings, and production integration remain pending.
...
- A known real private harmful aura, optional explicit secrecy/`NeverSecret` tests, the DEBUFFS filtering product decision, broader native item-enchantment/effect coverage, LEFT/RIGHT/ABOVE and arbitrary graph/cycle parity, managed `anchorsShown`, full ENCHANTMENTS bounds, remaining behavior/filter integration, legacy-only UI cleanup, production cutover, rollback policy, and removal of the temporary comparison infrastructure and legacy scanner remain future work.
