...
| Remaining production work | The duplicate Config refresh/application audit, historical-default cleanup, atomic production rename, internal managed-renderer terminology cleanup, and redundant BUFFS/DEBUFFS Sort-button removal are complete. Diagnostic-retention review is the next cleanup task. Any evidence-driven frame-name migration decision, final Config polish, library/licensing review, and release metadata remain separate later tasks. |
...
12. Should the six intentionally preserved `Prototype`-containing runtime frame names ever migrate for a concrete compatibility or ownership reason? Defer cosmetic migration; diagnostic retention/removal remains a separate audit.
