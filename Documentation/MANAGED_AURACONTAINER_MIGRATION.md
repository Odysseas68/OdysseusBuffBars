...
| Persistent position and full configuration integration | Partial. Independent supported SCREEN roots have runtime-validated drag persistence, SavedVariables-only position ownership, lock/combat behavior, legacy synchronization, reset/reload, managed `anchorsShown` visibility, and comparison interaction. LEFT/RIGHT/ABOVE, arbitrary graph/cycle parity, remaining behavior/filter settings, legacy-only control cleanup, and production integration remain pending. |
...
Earlier runtime validation passed without `/reload` for live LEFT/RIGHT icon side, group alpha/scale, BUFFS/DEBUFFS saved sort and maximum counts, mixed-scale chaining, BUFFS/ENCHANTMENTS growth direction, all supported B/D/E SCREEN and BELOW combinations, repeated live SCREEN/BELOW switching, independent B/D/E SCREEN dragging, anchored-child following, anchored movement refusal/warning, real legacy synchronization, reset, lock and combat restrictions, and comparison-mode interaction. Development visibility/comparison coverage included managed-only, synchronized overlapping, comparison ON/OFF, both options enabled, mixed root/dependent topology, dragging without persistent drift, and side-by-side behavior. Managed header visibility also passed live ON/OFF testing. The stale-login anchor failure was diagnosed separately from header visibility: despite correct saved topology, all three named hosts reported user-placed ownership and D/E were restored to stale screen points. After the movable-first ownership fix, two full exit/login cycles reported `B userPlaced false D false E false`; in the saved BUFFS SCREEN -> DEBUFFS BELOW -> ENCHANTMENTS BELOW topology, dragging BUFFS moved D/E immediately before and after the second relogin without dropdown cycling. Earlier placement coverage for reload persistence, nondefault offsets, mixed growth/scale, managed growth/shrink, combat sizing/chaining, and native weapon-transition recovery remains valid. ENCHANTMENTS UP mode also passed reload persistence, mixed-source presentation, width/height/spacing/icon-side changes, loading and no-enchant transitions, native recovery without growth reset, and combat behavior. Within the supplied test scope, native managed presentation, tooltips, BUFF and weapon-enchantment cancellation, DEBUFF behavior, fishing-lure presentation/tooltip, aura updates, group chaining, and combat behavior continued working without reported regressions.
