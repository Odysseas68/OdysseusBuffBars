...
Phase A, managed AuraButton presentation, Phase B.2 dynamic self-sizing, native managed sorting, player-BUFFS whitelist/blacklist semantics, and automatic synchronization from the existing BUFFS filter editor are validated on the Retail 12.1 PTR. The isolated BUFFS, DEBUFFS, and ENCHANTMENTS prototypes pass Retail Live runtime comparison against the legacy visual presentation. The current configuration bridge live-synchronizes presentation geometry, LEFT/RIGHT icon placement, host scale/alpha, and all three growth directions; BUFFS/DEBUFFS additionally consume saved sort and maximum counts. Managed ENCHANTMENTS follows a deliberate 7+2+1 capacity and source-order policy, and bounded native-row recovery remains validated across world/loading transitions, including while configured UP. These remain parallel prototypes; broader configuration parity and production cutover are not complete.
...
| Live configuration synchronization | Runtime validated out of combat for font/color/geometry, `iconSide`, host scale/alpha, BUFFS/DEBUFFS sort and `maxBars`, and BUFFS/ENCHANTMENTS growth direction. DEBUFFS growth is implemented through the same supported path without equivalent direct real-HARMFUL coverage. |
| Persistent position and full configuration integration | Partial. Placement/position, remaining behavior/filter settings, legacy-only control cleanup, and production integration remain pending. |
...
- **Runtime-validated live OOC:** `fontSize`, `barColor`, `barBgColor`, `width`, `height`, `spacing`, `iconSide`, group `scale`, group `alpha`, BUFFS/DEBUFFS `sort`, BUFFS/DEBUFFS `maxBars`, and BUFFS/ENCHANTMENTS `growUp`.
...
- **Pending/research:** arbitrary placement/chaining, SCREEN positioning, BELOW beyond the validated chain, LEFT/RIGHT placement, ABOVE architecture, x/y and `anchorTo`/offset parity, timed/timeless parity, DEBUFF filter parity, native item-enchantment filtering/hiding parity, additional saved-override composition, legacy-only control cleanup, production cutover, and legacy removal.
...
Status: Core managed MainHand lifecycle, bounded transition recovery, dynamic semantic HELPFUL routing, the fishing-lure exception, visual parity, live presentation synchronization, and ENCHANTMENTS growth are validated on Retail Live. The managed 7+2+1 capacity and source-order policy is implemented. Direct OffHand/both-slot transition coverage, broader native parity, placement/position parity, and production integration remain pending.
