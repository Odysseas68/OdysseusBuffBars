...
| Remaining production work | The duplicate Config refresh/application audit, historical-default cleanup, atomic production rename, internal managed-renderer terminology cleanup, redundant BUFFS/DEBUFFS Sort-button removal, and diagnostic retention/removal audit are complete. `DumpKnownAuraTooltips` retention/removal remains deferred pending runtime evidence. Any evidence-driven frame-name migration decision, final Config polish, library/licensing review, and release metadata remain separate later tasks. |
...
12. Should the six intentionally preserved `Prototype`-containing runtime frame names ever migrate for a concrete compatibility or ownership reason? Defer cosmetic migration; the diagnostic retention/removal audit is complete, while `DumpKnownAuraTooltips` remains retained pending runtime evidence for any later removal.
