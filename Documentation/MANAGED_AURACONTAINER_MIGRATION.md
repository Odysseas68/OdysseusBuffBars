...
| Remaining production work | The duplicate Config refresh/application audit, historical-default cleanup, atomic production rename, internal managed-renderer terminology cleanup, redundant BUFFS/DEBUFFS Sort-button removal, and diagnostic retention/removal cleanup are complete, including retirement of the isolated fixed-ID tooltip/spell metadata inspector. Any evidence-driven frame-name migration decision, final Config polish, library/licensing review, and release metadata remain separate later tasks. |
