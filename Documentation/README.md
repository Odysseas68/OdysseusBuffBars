...
All three groups pass visual comparison against the legacy renderer. Managed HELPFUL ownership is resolved with precedence `hidden` -> explicit group override -> semantic enhancement route -> default BUFFS; destination whitelist/blacklist runs afterward and never establishes ownership. That same resolver now drives BUFFS/ENCHANTMENTS `Current group auras` from a fresh runtime-only snapshot of active readable player HELPFUL numeric spell IDs. BUFF-owned IDs appear only in B, ENCHANTMENTS-owned HELPFUL IDs only in E, and hidden IDs in neither; override save/delete moves ownership immediately without reload or aura churn. DEBUFF filtering, native item/lure per-ID filtering, final behavior/filter parity, and production cutover remain in progress.
...
The B/E filter editor's `Current group auras` section represents current active readable managed HELPFUL ownership, not a historical seen-aura cache. ENCHANTMENTS covers only semantically or explicitly routed HELPFUL AuraGroup IDs; it excludes native MainHand/OffHand providers and Fishing Lure. Destination filters may hide a rendered row without removing its owned current-list entry. When an aura expires it leaves the convenience list, while saved whitelist/blacklist entries and manual Spell ID support remain intact for inactive, hidden, historical, encounter, or test IDs. DEBUFFS current rows intentionally remain legacy-backed.
