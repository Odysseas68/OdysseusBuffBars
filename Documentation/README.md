...
All three groups now pass static visual comparison against the legacy renderer with shared `260 x 18` row/header styling, three-pixel spacing, full-coordinate icons, accepted text geometry, and group colors. Native AuraButton ownership, duration presentation, tooltips, sorting, supported cancellation, and managed self-sizing remain Blizzard-owned. Fishing Bobber routing changes only OBB's presentation of an ordinary HELPFUL aura. The synthetic `Fishing Lure` row uses event/API-driven state and a presentation-only timer; it shows the fishing-tool inventory tooltip and does not support cancellation. Existing configuration still changes only the legacy UI, so managed configuration synchronization, persistence, broader validation, and production cutover remain pending.
