...
- Treat MANAGED-only authority retirement as complete while keeping dormant backend/file cleanup and release metadata as separate work.
...
- `OdysseusBuffBars_Auras.lua` — dormant legacy scanner/cache backend, still loaded temporarily because managed Fishing Lure presentation uses its time formatter.
- `OdysseusBuffBars_Bars.lua` — dormant legacy ordinary-bar/secure-overlay backend; loaded by the TOC but never initialized.
