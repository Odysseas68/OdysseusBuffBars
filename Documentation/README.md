...
The legacy implementation uses direct `C_UnitAuras` scanning, addon-owned aura records, ordinary custom bars, and separate secure cancellation overlays. Renderer authority is now runtime-only and group-specific: BUFFS and ENCHANTMENTS default to `LEGACY`, while DEBUFFS defaults to `MANAGED`. The direct scanner remains authoritative only for legacy-authoritative groups.
...
Two temporary development settings support migration comparison. `Show Legacy BuffBars (Development)` (`showLegacyBars`, default `true`) controls only addon-owned legacy presentation for legacy-authoritative groups. `Legacy Comparison Mode (Development)` (`legacyComparisonMode`, default `false`) forces eligible legacy presentation visible and shifts each effective legacy SCREEN root horizontally by its group width plus 24 UI units. It preserves the established BUFFS/ENCHANTMENTS comparison behavior but cannot resurrect legacy DEBUFFS while DEBUFFS authority is `MANAGED`. The comparison offset is presentation-only and never enters real placement fields. These settings remain migration infrastructure, not renderer authority.
...
- `OdysseusBuffBars_ManagedPrototype.lua` — managed BUFFS, DEBUFFS, and ENCHANTMENTS containers, including production-authoritative DEBUFFS, shared static presentation, semantic HELPFUL routing, native weapon enchantments, and the fishing-lure exception.
