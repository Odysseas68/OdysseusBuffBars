...
All three groups pass visual comparison against the legacy renderer. The out-of-combat config bridge now live-synchronizes presentation, growth, supported placement, BUFFS/DEBUFFS sort and maximum counts, and the validated HELPFUL behavior/filter slice. Managed HELPFUL ownership is resolved once with precedence `hidden` -> explicit group override -> semantic enhancement route -> default BUFFS, then the destination whitelist/blacklist is applied only within that route. One authoritative compiler produces complete BUFFS and `HelpfulEnhancements` candidate descriptors and applies/snapshots both together, preventing a filter edit from clearing semantic routing. BUFFS supports ALL and TIMED_ONLY duration policies through Blizzard candidate filtering; DEBUFFS and ENCHANTMENTS intentionally always include all eligible durations. Arbitrary graph/cycle parity, DEBUFF filtering, native item/lure per-ID filtering, editor discovery cleanup, final behavior/filter parity, and production cutover remain in progress.
...
The existing filter editor's `Current group auras` rows remain a historical legacy scanner cache, not a live enumeration of effective managed ownership. A semantically routed or overridden aura can therefore remain listed under an old group after routing changes or reload even while the managed backend is correct. Manual Spell ID entry remains available; rebuilding editor population from current effective managed ownership is future UI/discovery work.
