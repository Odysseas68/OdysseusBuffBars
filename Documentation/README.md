...
The legacy implementation uses direct `C_UnitAuras` scanning, addon-owned aura records, ordinary custom bars, and separate secure cancellation overlays. It now remains only as rollback/development infrastructure. Renderer authority is runtime-only and mode-level: normal startup enters `MANAGED` for BUFFS, DEBUFFS, and ENCHANTMENTS together after non-destructive preflight. Historical unsupported duration/topology is interpreted through a copied runtime-only managed state without rewriting SavedVariables; structural/capability failures still leave startup safely STAGED.
