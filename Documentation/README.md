...
The legacy implementation uses direct `C_UnitAuras` scanning, addon-owned aura records, ordinary custom bars, and separate secure cancellation overlays. It now remains only as rollback/development infrastructure. Renderer authority is runtime-only and mode-level: normal startup enters `MANAGED` for BUFFS, DEBUFFS, and ENCHANTMENTS together only after protected managed initialization reports READY and the non-destructive preflight succeeds. Historical unsupported duration/topology is interpreted through a copied runtime-only managed state without rewriting SavedVariables. Fatal managed initialization instead activates fresh runtime-only LEGACY presentation for the session, reports one ERROR, and leaves SavedVariables unchanged; this is temporary safety-net policy pending authority retirement.

The managed module object always exists. `ManagedPrototype:IsReady()` returns successful readiness or a retained failure reason across explicit uninitialized, initializing, ready, and terminal failed lifecycle states. Startup validates static Retail capabilities where possible, then keeps provisional hosts, headers, containers, lure, events, and initial AuraButton batches hidden/disabled within one protected construction boundary. READY requires complete B/D/E infrastructure, required constructed methods, successful initial paired BUFFS/ENCHANTMENTS descriptors, a complete applied paired snapshot, and successful presentation commit. Fatal containment invalidates delayed callbacks, unregisters or gates addon-owned managed event work, stops dragging and lure timer activity, and disables/hides surviving frames as best effort; it does not destroy WoW frames. Same-session reconstruction and MANAGED/STAGED are rejected until `/reload`.

Normal production `/reload` returned to MANAGED with normal B/D/E presentation and unchanged managed-to-legacy position synchronization. Temporary diagnostic injection runtime-tested missing capability, failure after provisional DEBUFFS construction, and initial ENCHANTMENTS descriptor failure. Each path produced exactly one managed-renderer ERROR, retained FAILED readiness, rejected MANAGED/STAGED, and entered LEGACY without reported duplicate managed presentation or unexpected Lua/taint/blocked-action errors. The temporary injection infrastructure and strings were completely removed before checkpointing, and a subsequent clean `/reload` returned to MANAGED.
...
ABOVE is intentionally unsupported in the managed architecture and planned for retirement with the legacy renderer. It requires the child's dynamic bottom/full visible height, but no safe public callback or content-height contract exists for the Blizzard-owned self-sizing container, and Fishing Lure sits outside ENCHANTMENTS container bounds. Config no longer offers ABOVE. Historical `ABOVE` remains stored unchanged and is interpreted through the canonical copied runtime fallback until the user explicitly chooses a supported placement.
...
The former `showLegacyBars` and `legacyComparisonMode` fields remain defaulted and preserved as dormant history/compatibility data, but their Config controls and runtime presentation effects were removed in Phase 1. Runtime authority alone now decides eligible legacy presentation.
