...
All three groups pass visual comparison against the legacy renderer. The out-of-combat config bridge now live-synchronizes font/color/geometry, LEFT/RIGHT icon placement, ordinary-host scale/alpha, saved growth direction, BUFFS `SCREEN` coordinates, and the exact default `BUFFS SCREEN -> DEBUFFS BELOW BUFFS -> ENCHANTMENTS BELOW DEBUFFS` chain. BUFFS and ENCHANTMENTS growth are directly runtime validated; DEBUFFS uses the same supported FlowLayout path without equivalent direct real-HARMFUL coverage. BUFFS and DEBUFFS additionally consume live saved sort and `maxBars`; prototype sort buttons remain temporary overrides until a later config apply reasserts saved sort. Managed placement reanchors ordinary hosts against actual self-sized container bounds, never the AuraContainers themselves. ENCHANTMENTS intentionally ignores legacy global sort/`maxBars`; arbitrary placement graphs/directions, drag/lock/anchor-visibility parity, remaining behavior/filter settings, persistence, and production cutover remain in progress.

Saved BUFFS coordinates continue to describe the logical stack top-left. The managed BUFFS host translates them by the four-pixel host padding and fixed 18-pixel header plus four-pixel first-row gap. BELOW placement uses the saved child offset against the parent managed container, with only the same four-pixel horizontal host correction and no header-height subtraction. `Reset Positions` retains its existing SavedVariables values but now completes through one `Config:Apply()` path. A managed empty container may collapse to `1 x 1`; chained groups follow that real bound rather than a fabricated legacy minimum. The ordinary Fishing Lure footer remains outside the ENCHANTMENTS container, which is terminal in the supported graph.
