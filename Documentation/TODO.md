...
- [ ] Remove the temporary immutable renderer/refresh Config compatibility façades only when Config no longer needs them; preserve SavedVariables compatibility.
...
The MANAGED-only authority migration, managed Fishing Lure formatter extraction, and Bars/secure-overlay retirement are complete and runtime validated: READY-or-FAILED startup, fail-closed failure handling, authority/API retirement, managed-only refresh/event ownership, managed-drag rollback synchronization removal, production runtime validation, the post-retirement CAPABILITY test, lure apply/remove/restore validation, the Bars retirement audit, narrow backend/TOC deletion, nil `OdysseusBuffBars.Bars`, and normal managed behavior without an observed duplicate/error/taint/blocked-action/restricted-layout regression are recorded. The read-only Auras/Engine audit, narrow file/TOC deletion, and focused deletion-proof `/reload` are also complete without Core, Config, ManagedPrototype, SavedVariables, or schema changes; `OdysseusBuffBars.Engine` was nil, `OBB.auraData` remained an empty table, `OBB.filterAuraRows` remained nil, normal MANAGED B/D/E presentation continued without legacy presentation, and no OBB Lua error was observed. Next work remains deliberately separate and dependency-ordered: a read-only Core bootstrap / Config compatibility fallback audit; Config façade and historical-field review; managed production renaming and terminology cleanup; final Config polish; media/library review; remaining targeted/opportunistic runtime coverage; the future `OdysseusDebugConsole`; licensing review; and version/build/release metadata.
