...
- [x] Complete the final managed-vs-legacy parity audit and all-managed B/D/E production authority cutover; preserve the historical rollback validation and raw SavedVariables as chronology/compatibility data.
- [x] Make ENCHANTMENTS Sort/Maximum Bars truthful by showing fixed managed Sort and disabling both controls in MANAGED while preserving historical stored values.
- [x] Finalize managed ENCHANTMENTS as broad/source-owned with no destination per-ID filtering across `HelpfulEnhancements`, MainHand, OffHand, or Fishing Lure; preserve historical filter tables as compatibility data.
...
- [x] Finalize managed DEBUFFS as intentionally broad/unfiltered, remove its Whitelist/Blacklist control, and preserve historical filter tables as compatibility data.
...
- [x] Cut over BUFFS, DEBUFFS, and ENCHANTMENTS together to MANAGED production authority on normal startup, with copied compatibility interpretation for unsupported historical configuration and no duplicate legacy display.
- [x] Historically validate production configuration, combat behavior, mode-aware refresh, LEGACY/STAGED rollback, reload restoration, and SavedVariables preservation before authority retirement.
- [x] Perform the read-only Auras/Engine retirement audit covering Config fallback use of `OBB.auraData`/filter rows, Engine namespace external-callability, Core bootstrap state, and dead cache/scanner ownership. The audit proved a narrow file/TOC deletion safe without Core, Config, ManagedPrototype, or SavedVariables changes.
- [x] Complete the read-only Bars/secure-overlay retirement audit, including dormant external-callability and secure overlay ownership.
- [x] Remove the temporary legacy visibility/comparison controls and presentation-offset path while preserving the defaulted SavedVariables as dormant compatibility/history fields at that checkpoint. The later authority-retirement slice removed the remaining operational rollback path.
...
- [x] Retire LEGACY/STAGED authority and direct-scanner fallback; make production startup READY-or-FAILED with one fail-closed managed renderer.
- [x] Remove authority tables, per-group/mode state, setter APIs, transition/preparation code, and mutable accessors. Retain only the immutable `GetRendererAuthorityMode() -> "MANAGED"` Config compatibility façade.
- [x] Runtime-validate the final fail-closed policy through one temporary CAPABILITY injection, then remove the hook, verify both diagnostic strings absent, restore exact production blobs, and pass a final clean `/reload`.
- [x] Extract the managed Fishing Lure time formatter into ManagedPrototype without changing lure discovery/events/timer/readiness behavior; runtime-validate appearance, stable approximately 10-minute countdown, fishing-pole removal, and restoration, and statically verify the 90-second, 5400-second, and 129600-second boundaries.
- [x] Audit and simplify Core event ownership after rollback retirement: remove legacy `UNIT_AURA`, `WEAPON_ENCHANT_CHANGED`, and `WEAPON_SLOT_CHANGED` registration/handling while preserving Config combat and Blizzard-frame/Edit Mode behavior.
- [x] Remove managed-drag to dormant legacy-position synchronization while preserving inverse SCREEN translation, applied managed placement state, synthetic-fallback refusal, anchored-group behavior, combat interruption, and normal SavedVariables persistence.
- [x] Remove the dormant legacy Bars renderer and secure overlay implementation, including its externally callable construction/update paths, by deleting `OdysseusBuffBars_Bars.lua` and its TOC entry without changing Core, Auras, Config, ManagedPrototype, or SavedVariables; runtime-validate a fresh `/reload`, nil `OdysseusBuffBars.Bars`, normal managed routing/presentation/cancellation/placement/refresh behavior, and no observed duplicate/error/taint/blocked-action/restricted-layout regression.
- [x] Runtime-validate the completed isolated deletion of `OdysseusBuffBars_Auras.lua` and its TOC entry: fresh `/reload` retained normal MANAGED B/D/E with no legacy presentation or observed OBB Lua error; `OdysseusBuffBars.Engine` was nil, `OBB.auraData` remained an empty table, and `OBB.filterAuraRows` remained nil. Routing/filter/editor/combat/portal behavior retains its previously established coverage and is not claimed as newly executed for this checkpoint.
- [x] Audit and remove the dead Core `OBB.groups`, `OBB.bars`, and `OBB.auraData` bootstrap tables plus Config's guarded `OBB.filterAuraRows`/`OBB.auraData` current-row fallback and private display helper. Preserve managed ID 1/3 discovery, persistent filters, candidate-filter application, overrides, and SavedVariables. Focused `/reload` validation confirmed `groups`, `bars`, `auraData`, and `filterAuraRows` all nil; no regression was reported.
- [x] Make the ENCHANTMENTS fixed Sort/Max Config state derive directly from group 3, remove the temporary immutable `GetRendererAuthorityMode()` Core façade without replacement, and runtime-validate that all three former authority APIs are nil while normal managed B/D/E, ENCHANTMENTS Config state, and `/obb refresh` remain correct; no regression was reported.
- [x] Complete the separate read-only BlizzardResearch review of the updated Live `Blizzard_AuraContainerUtil.lua` and `Blizzard_PrivateAurasUI.lua`; the audited changes did not demonstrate a compatibility impact requiring OBB production changes.
- [x] Audit the managed-only `RefreshAll()`/`RefreshAuras()` callers, retain `RefreshAll()` unchanged as the central coordinator, remove only the redundant `RefreshAuras()` alias, and redirect slash refresh plus Config `Refresh Auras` with their exact existing reason strings. Runtime deletion proof confirmed `RefreshAll` was a function and `RefreshAuras` was nil; normal managed B/D/E, both refresh paths, ordinary Config Apply, and the in-combat no-op passed with no reported regression.
- [x] Audit and clean up duplicate Config refresh/application paths. The six Config Apply/filter/override sites now suppress their direct managed follow-up after successful `RefreshAll()` transactions while retaining the existing Apply/candidate call as a failed-transaction fallback. Focused runtime validation passed normal managed presentation, refresh, Config Apply, filter/override mutation, and semantic routing behavior with no reported regression; the fallback retry path was preserved statically and was not deliberately failure-injected.
- [x] Complete the read-only dormant renderer-era defaults audit and prove `showLegacyBars` and `legacyComparisonMode` have no production Lua reader, writer, Config dependency, ManagedPrototype dependency, migration requirement, or active compatibility role.
- [x] Remove `showLegacyBars` and `legacyComparisonMode` from the Core defaults only, so new/default databases no longer receive them while existing serialized copies and other unknown historical fields remain preserved without migration, purge, schema change, or destructive rewrite.
- [x] Focused-runtime-validate the defaults-only cleanup: `/reload`, managed B/D/E, Config, and `/obb refresh` passed without a reported duplicate, Lua, or restricted-layout regression; the existing DB retained historical `showLegacyBars=false` and `legacyComparisonMode=true` values with no current runtime effect. No clean/new-database runtime test is claimed.
- [ ] Review obsolete historical Config fields/UI only when explicitly intended; preserve unknown and historical SavedVariables rather than folding cleanup into renderer retirement.
- [x] Complete the atomic production rename from `OdysseusBuffBars_ManagedPrototype.lua` / `OBB.ManagedPrototype` to `OdysseusBuffBars_Managed.lua` / `OBB.Managed`; retire the undocumented `OBB.managedAuraPrototype` alias without a compatibility shim; preserve TOC load position, SavedVariables/schema, and runtime semantics; and pass static plus focused runtime validation.
- [x] Clean normal internal managed-renderer terminology without erasing historical prototype/PTR/rollback chronology. Rename the capability gate, default aura-sort constant, lifecycle mutation guard, B/D/E construction helpers, audited runtime parameters, two current-state comments, and two internal strings without behavior change. Preserve exactly six `Prototype`-containing global runtime frame names because engine-side persistent frame identity/layout-cache behavior does not justify cosmetic migration; add no SavedVariables migration or compatibility shim. Focused `/reload` validation retained correctly positioned B/D/E, normal managed behavior, Config sorting, and visible BUFFS/DEBUFFS Sort-button label synchronization with no observed runtime regression.
- [x] Remove the two visible BUFFS/DEBUFFS Sort buttons while preserving Config-owned BUFFS/DEBUFFS sorting, existing SavedVariables sort values, and ENCHANTMENTS' fixed `TIMELEFT` behavior. Only button-specific creation, click, cycle, label, and reference plumbing was removed. Focused runtime validation confirmed both buttons absent, Config-controlled sorting functional, normal managed presentation correct, fixed ENCHANTMENTS sorting intact, and no observed regression.
- [x] Complete the diagnostic retention/removal audit and smallest safe cleanup: remove the permanently disabled automatic-routing debug gate/trace layer and unused module-table callback/event-frame mirrors while preserving production-local lifecycle state, manual helpful-enhancement/Fishing Lure diagnostics, unexpected failure output, compatibility reporting, and fatal failure reporting. Focused runtime validation passed `/reload`, normal B/D/E presentation, automatic routing, the manual diagnostic, and `/obb refresh`; `1234969` Ethereal Augmentation classified as `AUGMENT_RUNE`, `432021` Flask of Alchemical Chaos classified as `FLASK_PHIAL`, the final routing set contained two IDs, synchronization was already current, and no regression was observed.
- [x] Retire the isolated `DumpKnownAuraTooltips` research inspector and its four private helpers after a narrow read-only audit proved they had no production, Config, SavedVariables, lifecycle, recovery, refresh, native-enchant, Fishing Lure, or retained-diagnostic dependency. Preserve the shared diagnostic utilities and historical tooltip/spell metadata research evidence.
- [ ] Defer migration of the six intentionally preserved `Prototype`-containing runtime frame names unless a concrete future compatibility or ownership reason appears.
- [x] Establish the first post-v1.0 Config shell from the legacy `/ous` Midnight visual family and integrate the production logo at the top of a scrollable General page without changing control behavior, routing, or SavedVariables.
- [x] Replace the interim literal Config slider buttons with Retail's native `MinimalSliderWithSteppersTemplate`, retain 46-pixel same-row numeric entry, tighten the General upper spacing into one three-checkbox row, finalize the `680 x 610` minimum, trim the obsolete fixed General scroll extent to its visible content plus 20 pixels, and keep the complete BUFFS page visible. Retail LIVE testing accepted native slider rendering/dragging/steppers, same-row value boxes, the General layout and checkbox fit, the final minimum size, tightened bottom scrolling, and scrollbar auto-hide behavior.
- [ ] Continue detailed page-by-page Config polish after the shell foundation; do not reopen unsupported topology or redesign runtime behavior as part of visual work.
- [x] Add one global LibSharedMedia status-bar texture setting shared by BUFFS/DEBUFFS/ENCHANTMENTS, with safe missing-media fallback and immediate managed application.
- [x] Add one global LibSharedMedia font-face setting shared by BUFFS/DEBUFFS/ENCHANTMENTS while preserving independent per-group font sizes, count-size behavior, and the separate header font path.
- [x] Add compact bounded General-page texture/font preview pickers using the Retail ScrollBox/MinimalScrollBar pattern, actual media previews, and the verified Blizzard checkbox asset; runtime-validate scrolling, selection, immediate B/D/E application, and `/reload` persistence.
...
- [ ] Design a future development-only `OdysseusDebugConsole` addon/tool with multiline diagnostic command input, registered diagnostic commands, formatted output, PASS/WARN/FAIL assertions, named diagnostic/test groups, and copyable reports suitable for ChatGPT/Codex across OBB, OUS, and other addon projects. Initial non-goals: do not make it a generic arbitrary-Lua executor or a BugGrabber/BugSack replacement.
- [x] Add an MIT license scoped only to OdysseusBuffBars-owned code plus a factual third-party notice that leaves bundled component terms to their upstream projects and preserved source notices.
- [x] Complete first public-release repository housekeeping: public TOC wording, user-facing README, canonical public changelog policy, manual GitHub Release ZIP procedure, and current-state documentation synchronization.
- [x] Set version `1.0.0`, build date `2026-08-28`, TOC-backed runtime version authority, and the read-only Config General-page release information.
- [x] Prepare the fuller canonical public `OdysseusBuffBars v1.0.0` changelog entry and synchronize the first-release documentation checkpoint.
- [x] Complete the final static v1.0.0 release validation with PASS: every established LuaCheck baseline matched, `git diff --check` passed, release state was coherent, and no static release blocker was found.
- [x] Complete the final v1.0.0 in-game sanity test on LIVE only with PASS: the addon was reported working correctly, no release-blocking runtime issue was found, and no PTR release-gate test was required.
- [x] Create the final documentation-only v1.0.0 release commit after the static and LIVE gates passed.
- [x] Task 8: create and push the annotated `v1.0.0` tag.
- [x] Task 9: create the curated installable ZIP and GitHub Release.
- [x] Task 10: perform post-release verification.
- [x] Add the post-v1.0 LibDataBroker/LibDBIcon minimap launcher, independent Retail Addon Compartment entry, shared open-only Config path, persistent minimap state, and production artwork; the user tested the implementation in WoW Retail and reported that everything worked correctly.
- [ ] Add broader localization support for semantic enhancement recognition; current automatic classification is designed and tested around readable English aura/spell text, with manual Spell ID routing/overrides as the existing fallback.

The MANAGED-only authority migration, managed Fishing Lure formatter extraction, Bars/secure-overlay retirement, Auras/Engine retirement, legacy runtime-cache cleanup, renderer-authority query-façade retirement, `RefreshAuras()` compatibility-alias retirement, successful-path Config refresh deduplication, dormant renderer-era defaults retirement, atomic production rename to `OdysseusBuffBars_Managed.lua` / `OBB.Managed`, internal managed-renderer terminology cleanup, redundant BUFFS/DEBUFFS Sort-button removal, diagnostic retention/removal cleanup, global LibSharedMedia texture/font integration, updated Live AuraContainer/private-aura source review, first public-release repository housekeeping, v1.0.0 version/build metadata, fuller first-release documentation, final static validation PASS, final LIVE sanity PASS, final documentation-only release commit, annotated tag, curated GitHub Release, post-release verification, and post-v1.0 launcher/artwork integration are complete and validated to their recorded boundaries. The undocumented lowercase alias was retired without a compatibility shim or SavedVariables/schema change. Historical evidence remains explicit: at the Auras-retirement checkpoint `OBB.auraData` was an empty table and `OBB.filterAuraRows` was nil; the later focused cache-cleanup `/reload` confirmed `groups`, `bars`, `auraData`, and `filterAuraRows` all nil. Alias-retirement validation confirmed `RefreshAll` remained a function and `RefreshAuras` was nil. The sequencing checkpoint retained direct managed calls only as failure fallbacks. The defaults checkpoint preserved existing `showLegacyBars=false` and `legacyComparisonMode=true` copies while source/default inspection proved new/default databases no longer receive them. The remaining operational/field diagnostics are intentionally retained; any evidence-driven frame-name migration, final Config polish, remaining targeted/opportunistic runtime coverage, broader localization, and the future `OdysseusDebugConsole` remain pending.
