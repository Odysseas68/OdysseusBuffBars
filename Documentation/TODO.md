...
- [ ] Next, perform a read-only BlizzardResearch review of the updated Live `Blizzard_AuraContainerUtil.lua` and `Blizzard_PrivateAurasUI.lua`, compare their changes against current managed assumptions, and do not modify OBB unless the source diff demonstrates a real compatibility impact.
- [ ] Review the managed-only `RefreshAll()`/`RefreshAuras()` compatibility façades only in a later independent task while their Config/slash callers remain active.
...
The MANAGED-only authority migration, managed Fishing Lure formatter extraction, Bars/secure-overlay retirement, Auras/Engine retirement, legacy runtime-cache cleanup, and renderer-authority query-façade retirement are complete and runtime validated to their recorded boundaries. Historical evidence remains explicit: at the Auras-retirement checkpoint `OBB.auraData` was an empty table and `OBB.filterAuraRows` was nil; the later focused cache-cleanup `/reload` confirmed `groups`, `bars`, `auraData`, and `filterAuraRows` all nil, with no reported regression. Next work remains deliberately separate and dependency-ordered: updated Live AuraContainer/private-aura source review in BlizzardResearch; later refresh-façade review; historical-field review; managed production renaming and terminology cleanup; final Config polish; media/library review; remaining targeted/opportunistic runtime coverage; the future `OdysseusDebugConsole`; licensing review; and version/build/release metadata.
