...
- [ ] Separately perform a read-only BlizzardResearch review of the updated Live `Blizzard_AuraContainerUtil.lua` and `Blizzard_PrivateAurasUI.lua`, compare their changes against current managed assumptions, and do not modify OBB unless the source diff demonstrates a real compatibility impact.
...
The MANAGED-only authority migration, managed Fishing Lure formatter extraction, Bars/secure-overlay retirement, Auras/Engine retirement, legacy runtime-cache cleanup, renderer-authority query-façade retirement, `RefreshAuras()` compatibility-alias retirement, successful-path Config refresh deduplication, dormant renderer-era defaults retirement, atomic production rename to `OdysseusBuffBars_Managed.lua` / `OBB.Managed`, internal managed-renderer terminology cleanup, redundant BUFFS/DEBUFFS Sort-button removal, diagnostic retention/removal cleanup, and initial global LibSharedMedia texture/font integration with compact preview pickers are complete and runtime validated to their recorded boundaries. The undocumented lowercase alias was retired without a compatibility shim or SavedVariables/schema change. Historical evidence remains explicit: at the Auras-retirement checkpoint `OBB.auraData` was an empty table and `OBB.filterAuraRows` was nil; the later focused cache-cleanup `/reload` confirmed `groups`, `bars`, `auraData`, and `filterAuraRows` all nil. Alias-retirement validation confirmed `RefreshAll` remained a function and `RefreshAuras` was nil. The sequencing checkpoint retained direct managed calls only as failure fallbacks. The defaults checkpoint preserved existing `showLegacyBars=false` and `legacyComparisonMode=true` copies while source/default inspection proved new/default databases no longer receive them. Updated Live AuraContainer/private-aura source review remains separate BlizzardResearch work. The remaining operational/field diagnostics are intentionally retained; any evidence-driven frame-name migration remains deferred and separate from final Config polish, library/licensing review, remaining targeted/opportunistic runtime coverage, the future `OdysseusDebugConsole`, and version/build/release metadata.
