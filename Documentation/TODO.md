...
- [ ] Perform a read-only Auras/Engine retirement audit before removing the dormant scanner/cache/filter/classification backend or its TOC entry. Include Config fallback use of `OBB.auraData`/filter rows, Engine namespace external-callability, Core bootstrap state, and dead cache/scanner ownership.
...
- [ ] After Bars no longer depends on Engine, audit/remove the dormant `OdysseusBuffBars_Auras.lua` scanner/cache/filter/classification backend and its TOC entry as an isolated deletion.
