...
- [x] Remove the temporary legacy visibility/comparison controls and presentation-offset path while preserving the defaulted SavedVariables as dormant compatibility/history fields. The later authority-retirement slice removed the remaining operational rollback path.
...
The MANAGED-only authority migration, managed Fishing Lure formatter extraction, Bars/secure-overlay retirement, Auras/Engine retirement, legacy runtime-cache cleanup, renderer-authority query-façade retirement, `RefreshAuras()` compatibility-alias retirement, and successful-path Config refresh deduplication are complete and runtime validated to their recorded boundaries. Historical evidence remains explicit: at the Auras-retirement checkpoint `OBB.auraData` was an empty table and `OBB.filterAuraRows` was nil; the later focused cache-cleanup `/reload` confirmed `groups`, `bars`, `auraData`, and `filterAuraRows` all nil. Alias-retirement validation confirmed `RefreshAll` remained a function and `RefreshAuras` was nil. The sequencing checkpoint retained direct managed calls only as failure fallbacks and passed focused managed presentation, refresh, Config Apply, filter/override mutation, and semantic routing validation without a reported regression. Next work remains deliberately separate and dependency-ordered: updated Live AuraContainer/private-aura source review in BlizzardResearch; historical-field review; managed production renaming and terminology cleanup; final Config polish; media/library review; remaining targeted/opportunistic runtime coverage; the future `OdysseusDebugConsole`; licensing review; and version/build/release metadata.
