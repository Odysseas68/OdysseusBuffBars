...
- [ ] Next, perform a separate read-only audit of duplicate Config refresh/application paths: `Config:Apply()` calling `RefreshAll()` then `ApplyConfiguration()` again, and filter/override mutations calling `RefreshAll()` then `RefreshCandidateFilters()` again. Do not change ordering or behavior before that audit.
...
The MANAGED-only authority migration, managed Fishing Lure formatter extraction, Bars/secure-overlay retirement, Auras/Engine retirement, legacy runtime-cache cleanup, renderer-authority query-façade retirement, and `RefreshAuras()` compatibility-alias retirement are complete and runtime validated to their recorded boundaries. Historical evidence remains explicit: at the Auras-retirement checkpoint `OBB.auraData` was an empty table and `OBB.filterAuraRows` was nil; the later focused cache-cleanup `/reload` confirmed `groups`, `bars`, `auraData`, and `filterAuraRows` all nil. Alias-retirement validation confirmed `RefreshAll` remained a function and `RefreshAuras` was nil, with both direct refresh paths, ordinary Config Apply, managed B/D/E, and the in-combat no-op passing without a reported regression. Next work remains deliberately separate and dependency-ordered: duplicate Config refresh/application caller audit; updated Live AuraContainer/private-aura source review in BlizzardResearch; historical-field review; managed production renaming and terminology cleanup; final Config polish; media/library review; remaining targeted/opportunistic runtime coverage; the future `OdysseusDebugConsole`; licensing review; and version/build/release metadata.
