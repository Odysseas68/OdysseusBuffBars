...
- [ ] Remove temporary scanner and tooltip containment with the dormant Auras backend only after dormant Bars no longer retains Engine formatter dependencies.
- [ ] Perform a read-only Bars/secure-overlay retirement audit, including dormant external-callability, before removing the ordinary Bars backend and its TOC entry.
...
- [ ] First audit and remove the dormant legacy Bars renderer and secure overlay implementation, including its externally callable construction/update paths, then remove `OdysseusBuffBars_Bars.lua` and its TOC entry.
...
The MANAGED-only authority migration and managed Fishing Lure formatter extraction are complete: READY-or-FAILED startup, fail-closed failure handling, authority/API retirement, managed-only refresh/event ownership, managed-drag rollback synchronization removal, production runtime validation, the post-retirement CAPABILITY test, and lure apply/remove/restore validation all passed. Next work remains deliberately separate and dependency-ordered: Bars/secure-overlay retirement audit and removal; Auras backend/file/TOC removal after Bars no longer depends on Engine; Config façade and historical-field review; managed production renaming and terminology cleanup; final Config polish; media/library review; remaining targeted/opportunistic runtime coverage; the future `OdysseusDebugConsole`; licensing review; and version/build/release metadata.
