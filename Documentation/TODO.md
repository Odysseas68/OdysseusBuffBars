...
- [ ] Remove the two visible BUFFS/DEBUFFS Sort buttons while preserving Config-owned BUFFS/DEBUFFS sorting, existing SavedVariables sort values, and ENCHANTMENTS' fixed `TIMELEFT` behavior. Remove only button-specific click, label, and update plumbing that becomes unused.
...
The MANAGED-only authority migration, managed Fishing Lure formatter extraction, Bars/secure-overlay retirement, Auras/Engine retirement, legacy runtime-cache cleanup, renderer-authority query-façade retirement, `RefreshAuras()` compatibility-alias retirement, successful-path Config refresh deduplication, dormant renderer-era defaults retirement, atomic production rename to `OdysseusBuffBars_Managed.lua` / `OBB.Managed`, and internal managed-renderer terminology cleanup are complete and runtime validated to their recorded boundaries. The undocumented lowercase alias was retired without a compatibility shim or SavedVariables/schema change. Historical evidence remains explicit: at the Auras-retirement checkpoint `OBB.auraData` was an empty table and `OBB.filterAuraRows` was nil; the later focused cache-cleanup `/reload` confirmed `groups`, `bars`, `auraData`, and `filterAuraRows` all nil. Alias-retirement validation confirmed `RefreshAll` remained a function and `RefreshAuras` was nil. The sequencing checkpoint retained direct managed calls only as failure fallbacks. The defaults checkpoint preserved existing `showLegacyBars=false` and `legacyComparisonMode=true` copies while source/default inspection proved new/default databases no longer receive them. Updated Live AuraContainer/private-aura source review remains separate BlizzardResearch work. Removing the visible BUFFS/DEBUFFS Sort buttons is the next code-cleanup boundary; diagnostic-retention review and any evidence-driven frame-name migration decision remain separate from final Config polish, media/library review, remaining targeted/opportunistic runtime coverage, the future `OdysseusDebugConsole`, licensing review, and version/build/release metadata.
