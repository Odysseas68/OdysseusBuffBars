...
Startup now preserves intentional independent DEBUFFS/ENCHANTMENTS SCREEN roots across serialization. Before recursive defaults fill missing fields, it records raw groups with an explicit placement and no saved parent, then restores that intentional nil `anchorTo` after defaults. This adds no persistent discriminator or schema field, does not change generic default filling or managed preflight, and does not repair malformed/unsupported graphs. Runtime validation passed D SCREEN, E SCREEN, both SCREEN, D BELOW B, E BELOW D, and Reset Positions across reload. The supported topology remains B as the SCREEN root; D may be SCREEN or BELOW/LEFT/RIGHT of B; E may be SCREEN or BELOW/LEFT/RIGHT of D. Reverse, arbitrary, cyclic, and B-as-child graphs remain unsupported and are rejected by managed preflight; no separate LEFT/RIGHT reload test is claimed by this checkpoint.
...
- Production migration — All three groups are managed-authoritative on supported startup; Phase 1 presentation cleanup and D/E SCREEN reload persistence are complete, while legacy authority/backend retirement remains future work
