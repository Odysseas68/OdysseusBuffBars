...
Supported public temporary-enchantment data supplies slot, enchant ID, remaining time, expiration state, charges, equipped-item tooltip context, native row identity, and cancellation slot, but no clean supported enchant-effect name. The addon deliberately does not tooltip-scrape, inspect private provider state, enumerate AuraButtons, or maintain a hardcoded enchant-name compatibility table.
