...
- Diagnostic cleanup — The retention/removal audit is complete; the permanently disabled automatic-routing trace gate and unused module-table lifecycle mirrors are removed while production-local lifecycle state, unexpected failure reporting, manual helpful-enhancement/Fishing Lure diagnostics, compatibility/fatal reporting, and the tooltip/spell metadata inspector remain
- Deferred cleanup — Reconsider `DumpKnownAuraTooltips` or the six protected runtime frame names only with runtime evidence or a concrete compatibility need; Config polish, media/library work, licensing, OdysseusDebugConsole, and release preparation remain separate
