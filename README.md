...
The normal out-of-combat configuration path live-synchronizes managed font/color/geometry, `iconSide`, host scale/alpha, group growth direction, header visibility through `anchorsShown`, and the validated placement set: BUFFS as SCREEN; DEBUFFS as SCREEN or BELOW BUFFS; and ENCHANTMENTS as SCREEN or BELOW DEBUFFS. All effective SCREEN roots support independent unlocked out-of-combat dragging with shared-coordinate persistence; anchored groups follow their supported parent and refuse direct movement. BUFFS/DEBUFFS additionally consume saved sort and `maxBars`. BUFFS and ENCHANTMENTS `growUp` are directly runtime validated, while DEBUFFS uses the same supported FlowLayout implementation without equivalent direct real-HARMFUL coverage. Managed placement moves ordinary hosts and follows actual self-sized container bounds; it does not reanchor or manually size AuraContainers. OBB SavedVariables remain the sole persistent position authority: managed hosts relinquish WoW user-placed ownership after startup and drag persistence. LEFT/RIGHT/ABOVE, arbitrary graph/cycle parity, remaining behavior/filter settings, UI cleanup, and production cutover remain pending.
...
- Configuration integration — Live presentation/growth, BUFFS/DEBUFFS sort/maximum counts, supported SCREEN/BELOW placement, SCREEN-root dragging, managed header visibility, and the development comparison workflow are validated; broader parity pending
