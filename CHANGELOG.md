...
- Centralized managed HELPFUL candidate-filter composition so semantic routes, existing hidden/group overrides, BUFF and ENCHANTMENTS destination filters, and BUFF duration policy compile into one complete BUFFS + `HelpfulEnhancements` snapshot. Both descriptors are applied together, and unchanged suppression now compares the complete effective state.
- Enforced route-before-destination-filter precedence: hidden wins, then explicit BUFFS/ENCHANTMENTS override, semantic enhancement routing, default BUFFS, and only then the selected destination's whitelist/blacklist. A destination whitelist can no longer create or change ownership.
- Applied ENCHANTMENTS spell-ID whitelist/blacklist only to the `HelpfulEnhancements` aura group. Native MainHand/OffHand providers and the ordinary Fishing Lure remain intentionally outside that filter surface and retain the 7+2+1 policy.
...
- Replaced legacy historical group ownership for BUFFS/ENCHANTMENTS `Current group auras` with a runtime-only snapshot of active readable player HELPFUL numeric spell IDs and the shared effective-route resolver.
- Made B/E editor ownership react immediately to hidden and BUFFS/ENCHANTMENTS override save/delete while remaining independent from destination whitelist/blacklist visibility. Removed active auras leave the convenience list without changing stored filter entries.
- Kept DEBUFFS current-aura population legacy-backed and kept ENCHANTMENTS native MainHand/OffHand providers plus Fishing Lure outside the managed HELPFUL convenience list.
