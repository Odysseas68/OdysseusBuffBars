...
- MANAGED startup preflight requires BUFFS as the SCREEN root, BUFF duration ALL or TIMED_ONLY, supported D/E SCREEN/BELOW/RIGHT/LEFT topology, initialized managed B/D/E infrastructure, and a successful coupled B/E descriptor application. Reject unsupported state without remapping or rewriting SavedVariables; remain in STAGED.
- Before recursive defaults, startup records raw saved groups that have an explicit placement but no serialized `anchorTo`; after defaults it restores that intentional nil parent. This preserves D/E SCREEN roots without a schema field or generic `CopyDefaults` change. Missing placement plus missing parent still receives the historical D->B / E->D defaults, while unsupported or contradictory explicit state remains available for managed preflight to reject.
...
## Phase Notes
- The production authority cutover is complete for BUFFS, DEBUFFS, and ENCHANTMENTS on supported startup. Runtime validation covers MANAGED startup, mode transitions/rollback, refresh behavior, Config truthfulness, combat, and unsupported-state STAGED fallback.
...
- Current phase: retain managed-only production plus rollback while handling legacy authority/scanner/bar/secure-overlay retirement, Config topology constraints, and any ManagedPrototype rename/split as separate reviewable cleanup work.
...
- Phase B.2 architectural direction and validated managed-placement constraints:
  ...
  - Managed dragging moves only ordinary addon-owned hosts. Persist the real shared SCREEN coordinates through the inverse host translation; legacy SCREEN roots use those same saved coordinates directly.
...
- D/E startup normalization passed reload tests for D SCREEN, E SCREEN, both SCREEN, D BELOW B, E BELOW D, and Reset Positions; valid tested states remain/start MANAGED and no longer trigger the former DEBUFFS unsupported-placement fallback. No explicit LEFT/RIGHT reload test is claimed. ABOVE, arbitrary/reverse/cyclic graphs, BUFFS-as-child, and future full-bounds chrome remain unsupported. Do not generalize the result without current Retail Live evidence or clearly scoped future PTR evidence when testing unreleased changes.
