...
- While MANAGED is authoritative, skip legacy B/D/E scanning/rendering, clear stale rows and secure-overlay state, and prevent comparison from resurrecting any legacy group. LEGACY and STAGED transitions prepare fresh legacy data before exposure.
...
- Current phase: checkpoint the validated all-managed architecture, then handle legacy scanner/bar/comparison/secure-overlay retirement and any ManagedPrototype rename/split as separate reviewable cleanup work.
...
- Phase B.2 architectural direction and validated managed-placement constraints:
  ...
  - `Show Legacy BuffBars (Development)` and `Legacy Comparison Mode (Development)` control only eligible legacy presentation for migration testing. They are not renderer authority and must never resurrect any managed-authoritative group.
...
- The supported SCREEN/BELOW/RIGHT/LEFT combinations, independent SCREEN-root dragging, SavedVariables-only host position ownership, and managed `anchorsShown` visibility have Retail Live validation for startup/live apply, persistence, reset, lock/combat handling, mixed growth/scale, combat sizing/chaining, comparison-mode interaction, empty managed bounds, and weapon-transition recovery. ABOVE, arbitrary graph/cycle parity, BUFFS-as-child, and future full-bounds chrome remain unsupported. Do not generalize the result without current Retail Live evidence or clearly scoped future PTR evidence when testing unreleased changes.
