...
- No renderer-authority query or setter API remains. Config derives ENCHANTMENTS' fixed managed Sort/Max state directly from group 3; preserved historical E sort/max values do not affect managed E runtime behavior.
- The managed module object always exists at `OBB.Managed`. `OBB.Managed:IsReady()` reports successful readiness or a retained reason across uninitialized, initializing, ready, and failed internal states. FAILED is terminal for the session; `/reload` is the only reconstruction attempt.
...
- The subsequent Core/bootstrap and Config cache cleanup removed `OBB.groups`, `OBB.bars`, `OBB.auraData`, `CanDisplayFilterText`, and the guarded `filterAuraRows`/`auraData` current-row fallback without changing SavedVariables or managed filtering. Focused runtime validation after `/reload` confirmed `groups`, `bars`, `auraData`, and `filterAuraRows` were all nil; no regression was reported.
- The renderer-authority query façade is retired. Runtime validation after `/reload` confirmed all three former authority APIs were nil, normal managed B/D/E presentation and `/obb refresh` remained correct, and ENCHANTMENTS Config still displayed fixed managed Sort with Sort and both Max Bars inputs disabled; no regression was reported.
- The `RefreshAll()`/`RefreshAuras()` caller audit is complete and the redundant `RefreshAuras()` compatibility alias is retired. Runtime validation confirmed `RefreshAll` remained a function, `RefreshAuras` was nil, normal managed B/D/E presentation and both direct user refresh paths remained correct, ordinary Config Apply still worked, and the existing in-combat framework-owned no-op behavior remained safe; no regression was reported.
- The duplicate Config refresh/application audit and cleanup are complete. Config captures `RefreshAll()` success at the six audited Apply/filter/override sites, skips the former direct managed follow-up after success, and retains `ApplyConfiguration("config apply")` or the existing candidate-filter path only as a failure fallback. Focused runtime validation passed with normal managed presentation, refresh, Config Apply, filter/override mutation, and semantic routing behavior; no regression was reported. The fallback retry path was preserved by implementation and static inspection, not deliberately failure-injected.
- The dormant renderer-era defaults audit and cleanup are complete. Core no longer defaults `showLegacyBars` or `legacyComparisonMode`, so new/default databases do not receive them. No production Lua reader or writer remains; existing serialized copies are preserved and ignored without migration, purge, schema change, or destructive rewrite. Focused runtime validation retained this user's historical `showLegacyBars=false` and `legacyComparisonMode=true` while `/reload`, managed B/D/E, Config, and `/obb refresh` remained normal with no reported duplicate, Lua, or restricted-layout regression.
- The atomic production rename is complete: `OdysseusBuffBars_ManagedPrototype.lua` became `OdysseusBuffBars_Managed.lua`, `OBB.ManagedPrototype` became `OBB.Managed`, and the undocumented `OBB.managedAuraPrototype` alias was retired without a compatibility shim. Static validation and focused runtime testing passed; the TOC load position, SavedVariables/schema, lifecycle, routing, filtering, placement, combat behavior, and diagnostics are unchanged.
- Internal managed-renderer terminology cleanup is complete. Normal Lua identifiers, audited helper parameters, two current-state comments, and two internal strings use production terminology; exactly six `Prototype` occurrences remain as intentionally unchanged global runtime frame names because engine-side persistent frame identity and layout-cache behavior are not sufficiently proven to justify cosmetic renaming. No SavedVariables migration, compatibility shim, or runtime behavior change was introduced. Focused `/reload` validation retained correctly positioned B/D/E, normal managed behavior, and Config sorting; the visible BUFFS/DEBUFFS Sort-button labels tracked Config changes, and no runtime regression was observed.
- The redundant visible BUFFS/DEBUFFS Sort buttons are removed. Sorting is Config-owned: saved B/D sort values remain intact and `ApplyConfiguration()` continues applying them through `ApplyManagedGroupSort()` and the managed containers; ENCHANTMENTS retains fixed `TIMELEFT` behavior. Focused runtime validation confirmed both buttons absent, Config-controlled sorting functional, normal managed presentation correct, fixed ENCHANTMENTS sorting intact, and no observed regression.
- The diagnostic retention/removal audit and its smallest behavior-neutral cleanup are complete. The permanently disabled automatic-routing debug gate/trace layer and unused module-table mirrors for callback generation and the two event frames are removed; the underlying lifecycle generations, local event frames, pending state, quiet-turn work, combat retries, manual helpful-enhancement/Fishing Lure diagnostics, tooltip/spell metadata inspector, compatibility reporting, and fatal failure reporting remain. Focused runtime validation passed `/reload`, normal B/D/E presentation, automatic routing, the manual routing diagnostic, and `/obb refresh` without an observed regression; the diagnostic classified `1234969` Ethereal Augmentation as `AUGMENT_RUNE` and `432021` Flask of Alchemical Chaos as `FLASK_PHIAL`, found two routed IDs, and reported the set already synchronized. `DumpKnownAuraTooltips` retention/removal and any future frame-name migration remain deferred unless runtime evidence or a concrete compatibility need justifies them.
...
- Stable project roots:
  ...
  - `D:\Program Files\Blizzard\World of Warcraft\_retail_\Interface\AddOns\OdysseusBuffBars\` is the active addon working copy for Retail 12.1 Live development and runtime validation. Production identity is `OdysseusBuffBars_Managed.lua` and `OBB.Managed`; remaining prototype terminology is historical or belongs to the six intentionally preserved runtime frame names.
...
- Phase B.2 architectural direction and validated managed-placement constraints:
  ...
  - Lateral placement must not use an empty AuraContainer's physical width. RIGHT uses the managed-runtime-owned applied parent logical width; LEFT uses the managed-runtime-owned applied child logical width. Preserve those widths in applied placement state so live configuration changes reapply without button enumeration, physical width reads, or fake minimum sizing.
  ...
  - `showLegacyBars` and `legacyComparisonMode` are no longer production defaults and have no production Lua reader or writer. New/default databases do not receive them; historical serialized copies remain preserved and ignored. Do not add a migration, purge, schema field, or destructive rewrite for those copies.
