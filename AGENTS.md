...
- The next code-cleanup task is removal of the two visible BUFFS/DEBUFFS Sort buttons while preserving Config-owned saved sorting and ENCHANTMENTS' fixed `TIMELEFT` behavior. Diagnostic retention/removal and any future frame-name migration decision remain separate tasks; defer frame-name migration unless a concrete need appears.
