...
- Blizzard-managed containers are the normal production authority for BUFFS, DEBUFFS, and ENCHANTMENTS. The mature direct-scanning implementation remains temporary rollback/development infrastructure pending a separate cleanup phase.
...
- Renderer authority is runtime-only and mode-level. Supported startup begins safely in `STAGED`, then attempts the existing MANAGED preflight/transaction; successful normal startup makes BUFFS, DEBUFFS, and ENCHANTMENTS managed-authoritative together.
- Legal modes are `STAGED` (B legacy, D managed, E legacy), `MANAGED` (B/D/E managed), and `LEGACY` (B/D/E legacy). Unsafe split combinations and independent per-group transitions are rejected.
- `SetRendererAuthorityMode("LEGACY")`, `SetRendererAuthorityMode("STAGED")`, and `SetRendererAuthorityMode("MANAGED")` are the out-of-combat, session-only rollback/development paths. Do not queue combat mutation or persist authority; `/reload` attempts MANAGED again.
- The managed module object always exists. `ManagedPrototype:IsReady()` reports successful readiness or a retained reason across uninitialized, initializing, ready, and failed internal states. FAILED is terminal for the session; same-session reconstruction and MANAGED/STAGED authority are rejected until `/reload`.
...
- MANAGED startup preflight requires effective BUFFS as the SCREEN root, effective BUFF duration ALL or TIMED_ONLY, supported effective D/E SCREEN/BELOW/RIGHT/LEFT topology, initialized managed B/D/E infrastructure, and a successful coupled B/E descriptor application. Historical unsupported raw duration/topology is evaluated into copied runtime-only effective settings; raw SavedVariables are never remapped or rewritten.
...
- Core consumes `Initialize()` and checks `IsReady()` before MANAGED. Fatal startup enters explicit runtime-only LEGACY, refreshes fresh legacy B/D/E presentation, reports one clear ERROR, and leaves SavedVariables unchanged. This LEGACY fallback is temporary cleanup-stage policy; the later authority-retirement phase should replace it with fail-closed managed presentation.
...
- While MANAGED is authoritative, skip legacy B/D/E scanning/rendering and clear stale rows and secure-overlay state. LEGACY and STAGED transitions prepare fresh legacy data before exposure, and authority-aware visibility exposes only legacy-authoritative groups.
...
- `/obb refresh` is mode-aware: it refreshes authoritative legacy groups where applicable and coordinates supported managed semantic/native/lure recovery out of combat; Blizzard-owned managed lifecycle remains framework-driven in combat.
...
- Retained legacy Bars guards cycles and self-anchors before `SetPoint`, but its local safe presentation fallback must not rewrite `anchorTo` or `placement`.
...
## Phase Notes
- The production authority cutover is complete for BUFFS, DEBUFFS, and ENCHANTMENTS on normal startup. Runtime validation covers MANAGED startup, mode transitions/rollback, refresh behavior, Config truthfulness, combat, loading, ordinary aura behavior, semantic routing, and available native enchant/lure behavior.
- Post-cutover cleanup Phase 1 is complete: the show-legacy/comparison Config UI and its SCREEN offset/save compensation are removed, while the defaulted fields remain dormant and LEGACY/STAGED rollback stays intact. The D/E SCREEN startup-normalization correction is also runtime validated.
...
- Managed capability/readiness and partial-initialization hardening is complete. Normal `/reload` returned to MANAGED with normal B/D/E presentation and unchanged managed-to-legacy position synchronization. Temporary diagnostic injection runtime-tested CAPABILITY, AFTER_DEBUFF_CONSTRUCTION, and INITIAL_E_DESCRIPTOR fatal paths; each retained FAILED readiness, rejected MANAGED/STAGED, entered LEGACY, and reported exactly one managed-renderer ERROR without reported unexpected Lua/taint/blocked-action errors. All temporary injection infrastructure and strings were removed before checkpointing.
- Current phase: retain managed-only production plus rollback. STAGED/LEGACY authority retirement, final fail-closed failure policy, managed-to-legacy position synchronization removal, legacy scanner/bar/secure-overlay retirement, lure-formatter extraction, and the `ManagedPrototype` production rename remain separate reviewable work.
- The mature direct-scanning implementation remains a rollback/compatibility backend. Do not treat it as the normal production path while MANAGED is authoritative, and do not delete it until rollback retirement is explicitly approved.
...
- Phase B.2 architectural direction and validated managed-placement constraints:
  ...
  - Managed dragging moves only ordinary addon-owned hosts. Persist the real shared SCREEN coordinates through the inverse host translation; legacy SCREEN roots use those same saved coordinates directly. Refuse drag persistence when the current managed SCREEN placement is a synthetic compatibility fallback.
  ...
  - The retired `showLegacyBars` and `legacyComparisonMode` SavedVariables remain dormant compatibility/history fields. They no longer have Config controls or affect presentation; runtime renderer authority alone determines eligible legacy presentation.
...
- The supported SCREEN/BELOW/RIGHT/LEFT combinations, independent SCREEN-root dragging, SavedVariables-only host position ownership, and managed `anchorsShown` visibility have Retail Live validation for startup/live apply, persistence, reset, lock/combat handling, mixed growth/scale, combat sizing/chaining, the historical comparison workflow, empty managed bounds, and weapon-transition recovery. Phase 1 runtime validation also covers MANAGED reload, absent development checkboxes, fresh LEGACY B/D/E, expected STAGED presentation, return to MANAGED, reload restoration, locked legacy-position preservation, unlocked managed-to-legacy rollback-position synchronization, and no observed duplicates/regression.
- D/E startup normalization passed reload tests for D SCREEN, E SCREEN, both SCREEN, D BELOW B, E BELOW D, and Reset Positions; valid tested states remain/start MANAGED and no longer trigger the former DEBUFFS unsupported-placement fallback. Phase 2 supplied tests also confirmed the constrained Config path and MANAGED/LEGACY/STAGED transitions. No explicit LEFT/RIGHT reload test or deliberate historical invalid-state injection is claimed. ABOVE, arbitrary/reverse/cyclic graphs, BUFFS-as-child, and future full-bounds chrome remain unsupported raw states handled only by the runtime compatibility bridge.
