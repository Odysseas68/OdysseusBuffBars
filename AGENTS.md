...
- The next cleanup is a separate read-only audit of duplicate Config refresh/application calls after `RefreshAll()`. Do not alter `Config:Apply()`, filter/override sequencing, direct `RefreshCandidateFilters()` calls, or duplicate `ApplyConfiguration()` behavior without that audit.
