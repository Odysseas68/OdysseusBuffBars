...
- The supported SCREEN/BELOW combinations and independent SCREEN-root dragging have Retail Live validation for startup/live apply, persistence, reset, lock/combat handling, mixed growth/scale, combat sizing/chaining, comparison-mode interaction, empty managed bounds, and weapon-transition recovery. LEFT/RIGHT/ABOVE, arbitrary graph/cycle parity, `anchorsShown` synchronization, and future full-bounds chrome remain unvalidated. Do not generalize the result without current Retail Live evidence or clearly scoped future PTR evidence when testing unreleased changes.
